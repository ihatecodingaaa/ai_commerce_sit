...
documented trust-boundary flaw in the actual application code. This is a
single, strict path: there is exactly one credential, one upload route,
and one sudo grant at every tier, with no alternate technique or shortcut
that skips a stage or reaches a later one without the previous ones.
...
which is instructor-only). `sudo -l` shows exactly one grant and nothing
else — there is only one lead to follow, deliberately.
